...
data <- read.csv("read_length_dist.post_trimming.human.csv", header=TRUE)
data <- read.csv("read_length_dist.contamination.mouse.csv", header=TRUE)
data <- read.csv("read_length_dist.post_trimming.mouse.csv", header=TRUE)

# Summarize the data to calculate mean and standard deviation
summary_data <- data %>%
  group_by(dataset) %>%
  summarise(across(21:46, list(mean = mean, sd = sd)))

# Reshape the data from wide to long format
summary_data_long <- summary_data %>%
  pivot_longer(cols = -dataset, names_to = c(".value", "column"), names_sep = "_")
...
mean_values <- data_long %>%
  filter(column == "mean") %>%
  select(dataset, header, mean_value = value)

test <- data_long %>%
  left_join(mean_values, by = c("header", "dataset")) %>%
...
result <- test %>%
  filter(column == "sd") %>%
  select(dataset, column, header, mean_value, value, lower, upper)

ggplot(test, aes(x = header, y = value, color = column)) +
  geom_line(data = subset(test, column == "mean"), color = 'black', aes(group = dataset)) +
...
//dataset_extracted <- test %>%
//  filter(dataset == "PRJNA507253")
