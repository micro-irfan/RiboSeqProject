...
data <- read.csv("read_length_dist.contamination.human.organs.csv", header=TRUE)
data <- read.csv("read_length_dist.contamination.human.organs.contaminants.csv", header=TRUE)
...
summary_data_long <- summary_data %>%
  pivot_longer(cols = -Organ, names_to = c(".value", "column"), names_sep = "_")
...
mean_values <- data_long %>%
  filter(column == "mean") %>%
  select(Organ, header, mean_value = value)

test <- data_long %>%
  left_join(mean_values, by = c("header", "Organ")) %>%
...
result <- test %>%
  filter(column == "sd") %>%
  select(Organ, column, header, mean_value, value, lower, upper)

list_of_organs <- c("Brain", 'ES', 'Fat', 'Fibroblast', 'Fibroblast - 507253')
#list_of_organs <- c('HA_EC', 'HCAEC', 'Hepatocytes', 'HUVEC', 'VSMC')

filtered_test <- subset(test, Organ %in% list_of_organs)

ggplot(filtered_test, aes(x = header, y = value, color = column)) +
  geom_line(data = subset(filtered_test, column == "mean"), color = 'black', aes(group = Organ)) +
  geom_errorbar(data = subset(filtered_test, column == "sd"), color = "black", alpha = 0.8, aes(x = header, ymin = lower, ymax = upper), width = 0.2) +
  facet_grid(vars(Organ)) +
  labs(x = "Read Length", y = "Fraction") +
  geom_vline(xintercept = c('28', '29', '30'), linetype = "dashed", color = "red", alpha = 0.5) + 
  theme_minimal() +
  scale_y_continuous(breaks = seq(0, 0.6, by = 0.1), limits = c(0, 0.45))
