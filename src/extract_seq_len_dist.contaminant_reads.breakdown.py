...
suffix= '.contaminants.fq.gz'

suffix_dict = {
    'mtDNA' : '.mtDNA.contaminants.fq.gz',
    'tRNA' : '.tRNA.contaminants.fq.gz',
    'rRNA' : '.rRNA.contaminants.fq.gz',
}
...
fastq_list_all = os.listdir(current_directory)

for contaminant, suffix in suffix_dict.items():
    fastq_list = [i for i in fastq_list_all if suffix in i]

    write_file = f'output.summary.{contaminant}.csv'
